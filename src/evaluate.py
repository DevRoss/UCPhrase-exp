...
class Evaluator:
    ...
    def evaluate_only(self, path_doc2cands):
        utils.Log.info(f'[Eval] {path_doc2cands}')
        doc2golds = consts.STEM_DOC2REFS
...
class SentEvaluator:
    ...
    def _to_char_offsets(sent):
        ...
        for splitsent in sent:
            ...
            spans.extend(_spans)        
        # assert all(x[2] == ''.join(text[x[0]: x[1]]) for x in spans)
        return spans
    ...
    def evaluate_only(self, path_decoded_doc2sents, *args):
        utils.Log.info(f'[Eval] {path_decoded_doc2sents}')
        doc2sents = utils.Json.load(path_decoded_doc2sents)
        num_predict = 0
        num_gold = 0
        ...
        pred_extra_spans = []
        gold_extra_spans = []
        for doc in tqdm(sorted(doc2sents.keys()), ncols=100, desc='eval'):
            sent = doc2sents[doc]
            spans = self._to_char_offsets(sent)
            pred_spans = set([(l, r, w) for l, r, w in spans])
            num_predict += len(pred_spans)
