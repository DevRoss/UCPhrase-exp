...
from preprocess import ZhBertProcessor
from preprocess import BaseAnnotator
from preprocess import WikiAnnotator
from preprocess import CoreAnnotator
from preprocess import ZhSupervisedAnnotator


class Experiment:
    rootdir = Path('../experiments_zh')
    ...
    def __init__(self):
        ...
        # preprocessor
        self.train_preprocessor = ZhBertProcessor(
    ...
    def predict(self, epoch, for_tagging=False):
        ...
        if for_tagging:
            test_preprocessor = ZhBertProcessor(
    ...
    def predict_only(self, epoch, for_tagging=False):
        test_preprocessor = None
        if for_tagging:
            test_preprocessor = ZhBertProcessor(
                path_corpus=self.data_config.path_tagging_docs,
        ...
        if for_tagging:
            ...
            evaluator = evaluate.SentEvaluator()
            print(f'Evaluate {path_decoded_doc2sents}')
            print(evaluator.evaluate_only(path_decoded_doc2sents))
        else:
            path_decoded_doc2cands = model_base.BaseModel.get_doc2cands(
                path_predicted_docs=path_predicted_docs,
                output_dir=dir_decoded,
            ...
            evaluator = evaluate.Evaluator()
            print(f'Evaluate {path_decoded_doc2cands}')
            print(evaluator.evaluate_only(path_doc2cands=path_decoded_doc2cands))
...
if __name__ == '__main__':
    ...
    exp.predict_only(epoch=best_epoch, for_tagging=True)
    # exp.predict_only(epoch=best_epoch, for_tagging=False)
